  ...
  return (
    <span className="inline-flex items-center gap-2">
      <span
        ...
        aria-hidden
      />
      {showLabel ? <span className="font-mono text-xs text-foreground/70">{sla.label}</span> : null}
    </span>
  );
}
  ...
  if (sla.level === 'none' || sla.slaHours == null || sla.ratio == null) {
    return (
      <div className="space-y-1">
        <SlaDot sla={sla} />
        <p className="font-mono text-xs text-foreground/40">Etapa sin SLA configurado</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between gap-2">
        <SlaDot sla={sla} />
        <span className="font-mono text-xs text-foreground/60">{Math.round(sla.ratio * 100)}%</span>
      </div>
      <div
...
export function SlaBadge({ sla }: { sla: SlaResult }) {
  if (sla.level === 'none') {
    return <span className="font-mono text-xs text-foreground/40">—</span>;
  }
  return (
    <span className="inline-flex items-center gap-2">
      <span
